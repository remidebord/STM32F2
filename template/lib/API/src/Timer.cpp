...
extern "C"
{
	char timUpdateAttach[14]; // 14 timers
...
void Ticker :: attach(void(*f)(void), int ms)
{
  ...
  if((m_tim == TIM1) || (m_tim == TIM8) || (m_tim == TIM9) || (m_tim == TIM10) || (m_tim == TIM11))
	{
		TIM_TimeBaseStructure.TIM_Prescaler = (((SystemCoreClock / APB2_PRESC) * 2) / 10000) - 1; // 120 MHz / 10000 (100 us)
	}
  else TIM_TimeBaseStructure.TIM_Prescaler = (((SystemCoreClock / APB1_PRESC) * 2) / 10000) - 1; // 60 MHz / 10000 (100 us)
...
void Ticker :: attach_us(void(*f)(void), int us)
{
  ...
  if((m_tim == TIM1) || (m_tim == TIM8) || (m_tim == TIM9) || (m_tim == TIM10) || (m_tim == TIM11))
	{
		TIM_TimeBaseStructure.TIM_Prescaler = (((SystemCoreClock / APB2_PRESC) * 2) / 1000000) - 1; // 120 MHz / 10000 (100 us)
	}
  else TIM_TimeBaseStructure.TIM_Prescaler = (((SystemCoreClock / APB1_PRESC) * 2) / 1000000) - 1; // 60 MHz / 10000 (100 us)
...
PwmOut :: PwmOut(GPIO_common GPIO_c, int freq, TIM_common PWM_c)
{
  ...
  /* Alternate function */
	...
	if((m_tim == TIM1) || (m_tim == TIM8) || (m_tim == TIM9) || (m_tim == TIM10) || (m_tim == TIM11))
	{
		m_timerPeriod = ((((SystemCoreClock / APB2_PRESC) * 2) / prescaler) / m_freq) - 1;
	}
  else m_timerPeriod = ((((SystemCoreClock / APB1_PRESC) * 2) / prescaler) / m_freq) - 1;
  ...
  TIM_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Set;
	...
	//channelPulse = ((m_dutyCycle*100) * (m_timerPeriod - 1)) / 100;
...
void PwmOut :: enable_irq(void)
{
  ...
  TIM_Cmd(m_tim,DISABLE);
	...
	/* IRQ Vector */
  if(m_tim == TIM1) irqVector = TIM1_UP_TIM10_IRQn;
	...
	else if(m_tim == TIM6) irqVector = TIM6_DAC_IRQn;
	else if(m_tim == TIM7) irqVector = TIM7_IRQn;
	else if(m_tim == TIM8) irqVector = TIM8_UP_TIM13_IRQn;
